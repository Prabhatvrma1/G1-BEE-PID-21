...
// JWT secret (token sign/verify ke liye)
// Real project mein yeh bhi ENV se aana chahiye
...
// Login endpoint ke liye rate limiter (brute force rokne ke liye)
const loginLimiter = rateLimit({
    windowMs: 10 * 60 * 1000, // 10 minutes
    max: 20, // 20 attempts per IP per 10min
    message: 'Too many login attempts, please try again later.',
    standardHeaders: true,
    legacyHeaders: false,
});

// Helper: user se JWT token generate karne ke liye function
function generateJwtForUser(user) {
    ...
    // Token 7 din ke liye valid rahega
...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        // User ko automatically login karayenge (session based for web)
        ...
        // Role ke hisaab se sahi dashboard par bhejenge (recruiter=admin, candidate=student)
...
// Login route - user ko login karane ke liye (web + API support)
router.post('/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        // Security ke liye new session create karenge (web clients ke liye)
        req.session.regenerate((err) => {
            ...
            // Agar client JSON expect kar raha hai (API use case)
            ...
            // Normal web login flow: redirect to appropriate area (recruiter=admin, candidate=student)
            const dashboard = user.role === 'recruiter' ? '/admin/companies' : '/student/home';
            res.redirect(dashboard);
        });
